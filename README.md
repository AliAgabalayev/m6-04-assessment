...
Download from [Google Drive](https://drive.google.com/drive/folders/1qeGvkaK7UkNMYoESQHxGbV4DRH8EgEb0?usp=drive_link) and place it at:
...
The `data/` folder is git-ignored — it must be downloaded locally.

### 4. Run the notebook end-to-end

```bash
jupyter notebook m6-04-assessment.ipynb
```

Run all cells from top to bottom (**Kernel → Restart & Run All**).

> **Note:** `data.yaml` is generated by Task 2 with an absolute `path:` pointing to your local clone. If you move the repo, re-run Task 2 cells to regenerate it before training.

The notebook will:
- Inspect the dataset and visualise bounding boxes (Task 1)
- Create 70/15/15 train/val/test splits and `data.yaml` (Task 2)
- Download `yolo26s.pt` pretrained weights and train for 30 epochs on GPU/CPU (Task 3)
- Evaluate on the test split and report mAP, P, R (Task 4)
- Visualise predictions and failure cases (Task 5)

Training takes roughly **15–20 min on an RTX 3060** or **60–90 min on CPU**.

### 5. Checkpoint

The best checkpoint is committed at:

```
runs/detect/runs/cats_v1/weights/best.pt   # yolo26n  ~5 MB
runs/detect/runs/cats_v2/weights/best.pt   # yolo26s  ~20 MB
```

To skip training and go straight to evaluation or prediction, load the checkpoint directly:
